...
if(isset($_POST['simpan'])) {
    ...
    if(empty($error)) {
        if($id != "") {
            $sql1 = "UPDATE portfolio SET title = '$judul', quote = '$kutipan', description = '$isi', date_of_contents = now() WHERE id = '$id'";
        } else {
            $sql1   = "INSERT INTO portfolio (title, quote, description) VALUES ('$judul', '$kutipan', '$isi')";
        }
        $q1 = mysqli_query($connect, $sql1);
        if($q1) {
