        ...
        <tbody>
            ...
            if ($keyword != '') {
                ...
                for ($x = 0; $x < count($array_keyword); $x++) {
                    $sqlSearch[] = "(fullname like '%" . $array_keyword[$x] . "%' or email like '%" . $array_keyword[$x] . "%' or telp like '%" . $array_keyword[$x] . "%' or subject like '%" . $array_keyword[$x] . "%' or message like '%" . $array_keyword[$x] . "%' )";
                }
                $sqlAdding = " where " . implode(" or ", $sqlSearch);
            }
            $sql1   = "SELECT * FROM contact $sqlAdding";
            ...
            ?>
                <tr>
                    ...
                    <td>
                        <a href="contact.php?op=delete&id=<?php echo $r1['id'] ?>" onclick="return confirm('Apakah yakin ingin menghapus data?')">
    ...
    <nav aria-label="Page navigation example" class="">
            <ul class="pagination">
                ...
                for($i = 1; $i <= $pages; $i++) {
                    ?>
                    <li class="page-item">
                        <a class="page-link" href="contact.php?keyword=<?php echo $keyword ?> &search=<?php echo $search?> &page=<?php echo $i?> "><?php echo $i ?></a>
                    </li>
                    <?php
                }            
                ?>
